...
from . import Contig, Scaffold, Parameter
...
from . import haplotypes as HR
from . import plots
from . import order_contigs
...
def Algorithm(G, G_prime, Contigs, small_contigs, Scaffolds, small_scaffolds, Information, param):
    ...
    print(str(nr_edges) + ' link edges created.', file=Information)
    print('Perform inference on scaffold graph...', file=Information)
    ...
    if param.extend_paths:
        ...
        print('Nr of contigs left: ', len(G_prime.nodes()) / 2.0, 'Nr of linking edges left:', len(G_prime.edges()) - len(G_prime.nodes()) / 2.0, file=Information)
        print('Number of gaps estimated by GapEst-LP module order_contigs in this step is: {0}'.format(param.path_gaps_estimated), file=Information)
...
def RemoveIsolatedContigs(G, Information):
    print('Remove isolated nodes.', file=Information)
    counter = 0
    for node in G.nodes():
        if node in G:
    ...
    print(str(counter) + ' isolated contigs removed from graph.', file=Information)
    return(G)
...
def remove_edges(G, G_prime, Information, param, node, score_chosen_obs, non_zero_score_removed_obs, edge_score_to_zero, record_decision):
    ...
    remove_zero_score_edges = [(node, item[1]) for item in zero_score_edges]
    ...
    if len(non_zero_score_edges) > 1:
        if non_zero_score_edges[-2][0] / non_zero_score_edges[-1][0] > 0.8: # lowered threshold due to the unceartinaty in scores
            print('SCORES AMBVIVALENT', non_zero_score_edges[-1][0], non_zero_score_edges[-2][0], file=Information)
            remove_non_zero_edges = [(node, item[1]) for item in non_zero_score_edges] # Edges that does not have a score of 0 but are not the highest scoring one in an ambigous region
        else:
            remove_non_zero_edges = [(node, item[1]) for item in non_zero_score_edges[:-1]] # Edges that does not have a score of 0 but are not the highest scoring one in an ambigous region
...
def RemoveAmbiguousRegionsUsingScore(G, G_prime, Information, param, plot):
    ...
    print('Remove edges from node if more than two edges', file=Information)
    #counter1 = 0

    link_edges = [x for x in G.edges(data=True) if 'score' in x[2]]
    ...
    print(' Number of edges in G before:', nr_edges_before, file=Information)
    print(' Number of edges in G after:', nr_edges_after, file=Information)
    try:
        print(' %-age removed edges:', 100 * (1 - (nr_edges_after / float(nr_edges_before))), file=Information)
...
#### After the proceure above, we hope that the graph is almost perfectly linear but we can still be encountering cycles (because of repeats or haplotypic contigs that has slipped through our conditions). Thus we finally search for loops
    print('Contigs/scaffolds left:', len(G.nodes()) / 2, file=Information)
    print('Remove remaining cycles...', file=Information)
    ...
    for cycle in list_of_cycles:
        print('A cycle in the scaffold graph: ' + str(cycle) + '\n', file=Information)
        print('A cycle in the scaffold graph: ' + str(cycle), file=Information)
        counter += 1
        for node in cycle:
            if node in G:
...
#                    del Scaffolds[scaffold_]
    print(str(counter) + ' cycles removed from graph.', file=Information)
...
### Remaining scaffolds are true sensible scaffolds, we must now update both the library of scaffold objects and the library of contig objects
    ...
    print('Nr of new scaffolds created in this step: ' + str(len(new_scaffolds_)), file=Information)
    ...
    for edge in new_scaffold_.edges_iter():
        ...
        if nr_links_:
            ...
            if high_score_path and score >= param.score_cutoff:
                ...
                small_scafs = [path[i] for i in [i for i in range(len(path) - 1) if i % 2 == 1]]
...
def permute_path(path, ctg_to_move, contig_after):
    ...
    path = [x for x in path if x[0] != ctg_to_move]
...
def calculate_path_LP(current_path, Scaffolds, small_scaffolds, observations, param, moved_forward=None, shifted=None):
    ...
    ctg_lengths_index_ordered = [x[1] for x in sorted(contig_lengths, key=lambda x: x[0])]
    ...
    if moved_forward and shifted:
        ...
        if mean_PE_obs > param.contamination_mean + 6 * param.contamination_stddev:
            ...
            if all(length in ctg_lengths_index_ordered for length in [670, 2093]) or all(length in ctg_lengths_index_ordered for length in [900, 3810]) or all(length in ctg_lengths_index_ordered for length in [2528, 591]) or all(length in ctg_lengths_index_ordered for length in [734, 257, 1548]):
                print('permuted link invalid. Continuing..', 'moved forward:', index_moved_forward, 'shifted:', index_shifted, file=param.information_file)
...
def estimate_path_gaps(Contigs, small_contigs, path,Scaffolds,small_scaffolds, G_prime, param):
    ...
    #print G_prime.subgraph(path).edges(data=True)
    sub_graph_reduced = [x for x in sub_graph.edges() if sub_graph[x[0]][x[1]]['nr_links'] != None and x[0][0] in sub_graph[x[0]][x[1]]]

    observations = dict([(x, [i+j for i,j in zip(sub_graph[x[0]][x[1]][x[0][0]], sub_graph[x[0]][x[1]][x[1][0]] )]) for x in sub_graph_reduced])
    sub_graph_small_to_large_ctgs = [x for x in sub_graph.edges() if sub_graph[x[0]][x[1]]['nr_links'] != None and x[0][0] not in sub_graph[x[0]][x[1]]]
    ...
    ## algm here
        ...
        ### O(n*(n-1)/2) solution idea ######
            ...
            max_nr_rounds = len(list(range(3, len(path) - 1, 2)))
            current_min_path = copy.deepcopy(path)
    ...
    path_dict = [(final_indexes_to_contigs[x[0].index],final_indexes_to_contigs[x[1].index], path_dict_index[x]) for x in path_dict_index]
    #print path_dict
    ...
    G_.add_edges_from(list(zip(final_path[::1], final_path[1::])))
...
def PROBetweenScaf(G_prime, Contigs, small_contigs, Scaffolds, small_scaffolds, param, dValuesTable, Information):
    ...
    if len(G.nodes()) / 2.0 > 10000:
        ...
        if len(small_scaffolds) / float(len(Scaffolds)) < 0.1 and not param.no_score:
            print("Did not enter path seartching algorithm between scaffolds due to too small fraction of small scaffolds, fraction were: ", len(small_scaffolds) / float(len(Scaffolds)), file=Information)
    ...
    if param.multiprocess and num_cores > 1:
        from . import workerprocess
        import heapq
        print('Entering ELS.BetweenScaffolds parallelized with ', num_cores, ' cores.', file=Information)
        start = time.time()
        # load up work queue
        ...
        while counter < nr_jobs:
            ...
            nr_processes += 1
            print('node nr', counter, 'to', counter + chunk - 1, 'added', file=Information)
        ...
        print("Elapsed time multiprocessing: ", elapsed, file=Information)
        ...
        iter_nodes = end.copy()
        print('Entering ELS.BetweenScaffolds single core', file=Information)
        all_paths_sorted_wrt_score = ELS.BetweenScaffolds(G_prime, end, iter_nodes, param)
        elapsed = time.time() - start
        print("Elapsed time single core pathfinder: ", elapsed, file=Information)
    ...
    print('{0} paths detected are with score greater or equal to {1} '.format(nr_paths, param.score_cutoff), file=Information)
    for sublist in reversed(all_paths_sorted_wrt_score):
        path = sublist[2]
        bad_links = sublist[1]
        score = sublist[0]
        path_len = sublist[3]
        if nr_paths <= 100000:
            print('Path: path length: {0}, nr bad links: {1}, score: {2} '.format((path_len - 2) / 2.0, bad_links, score), file=Information)
        ...
        if path[0][0] not in Scaffolds and path[-1][0] not in Scaffolds:
            try:
                ...
                if strt[0] == nd[0]:
                    print('Rare case (circular paths) detected and treated. ', file=Information)
                    continue
            except KeyError:
                pass
        ...
        if path[0][0] not in Scaffolds:
            if path[0] in start_end_node_update_storage:
                ...
                print('Beginning is already in path', file=Information)
                continue
        ...
        if path[-1][0] not in Scaffolds:
            if path[-1] in start_end_node_update_storage:
                ...
                print('End is already in path', file=Information)
                continue
        ...
        if nr_paths <= 100000:
            print('Path taken! path length: {0}, nr bad links: {1}, score: {2} '.format((path_len - 2) / 2.0, bad_links, score), file=Information)
